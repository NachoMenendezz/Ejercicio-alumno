...
void menuPrincipal(){
    ...
    while(true){
        ...
        cout<<"4 - BUSCAR LEGAJO"<<endl;
        cout<<"5 - BUSCAR FECHA"<<endl;
        cout<<"0 - SALIR"<<endl;
        cout<<"=============="<<endl;
...
void listarLegajo()
{
    Alumno obj;
    int legajo;
    cout<<"INGRESE EL LEGAJO DEL ALUMNO A LISTAR: ";
    cin>>legajo;
    int cantReg = contarAlumnos();
    ...
    for(int i = 0; i < cantReg; i++)
    {
        ...
        return;
    /// SI NO ENCUENTRA EL LEGAJO MUESTRA POR PANTALLA ESTE MENSAJE
}
...
int cantReg = contarAlumnos();
 for(int i = 0; i < cantReg; i++)
    {
        obj = leerAlumno(i);
        if (anio>obj.getAnioNacimiento()){
            if (mes>obj.getMesNacimiento())
            if (dia>obj.getDiaNacimiento())
                system("cls");
                obj.Mostrar();
                bool hubo=true;
                }
...
cout<<"NO EXISTEN ALUMNOS CON FECHA DE NACIMIENTO ANTERIOR A LA INGRESADA"<<endl;
    }
}
